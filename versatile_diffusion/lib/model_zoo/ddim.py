...
class DDIMSampler(object):
    ...
    def p_sample_ddim(self, x_info, c_info, t, index, 
        ...
        if unconditional_guidance_scale == 1.:
            ...
            e_t = self.model.apply_model(x_info, t, c_info, dependency_matrix, use_fca)
        else:
            x_in = torch.cat([x] * 2)
            t_in = torch.cat([t] * 2)
            ...
            x_info['x'] = x_in
            c_info['c'] = c_in
            e_t_uncond, e_t = self.model.apply_model(x_info, t_in, c_info, dependency_matrix, use_fca).chunk(2)
