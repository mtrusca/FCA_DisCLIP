...
class VD_v2_0(nn.Module):
    ...
    @torch.no_grad()
    def ctx_encode(self, x, which, constituents, use_disclip,
                   lal_dependencies, lal_tokens, n_samples, use_fca, seed, **kwargs):
        if which.find('vae_') == 0:
            return self.vae[which[4:]].encode(x, **kwargs)
        else:
            return self.ctx[which].encode(x, constituents, use_disclip,
                                          lal_dependencies, lal_tokens, n_samples, use_fca, seed, **kwargs)
    ...
    def apply_model(self, x_info, timesteps, c_info, dependency_matrix=None, use_fca=1):
        ...
        for ltype in i_order:
            if ltype == 'd':
                module = next(d_iter)
                h = module(h, emb, None, dependency_matrix, use_fca)
            elif ltype == 'c':
                module = next(c_iter)
                h = module(h, emb, c, dependency_matrix, use_fca)
