...
def get_lal_to_clip_idx_projection(lal_tokens, clip_tokens):
    result = {}
    lal_idx = 0
    import re
...
class CLIPTextContextEncoder(AbstractEncoder):
    ...
    def encode(self, text, constituents, use_disclip,
               lal_dependencies, lal_tokens, n_samples, use_fca, seed):
        ...
        outputs = self.model.text_model(input_ids=tokens) #prompt_embeds
        ...
        if use_disclip == 1:
            tokenized_sent = self.tokenizer.convert_ids_to_tokens(tokens[0], skip_special_tokens=False)
            g_sent_idx = tokenized_sent.index('<|endoftext|>')
            for const_n, (const_prompt, const_dep) in enumerate(constituents):
                const_ids = self.tokenizer(const_prompt, padding="longest", return_tensors="pt")["input_ids"]
                const_embeddings = self.model.text_model(const_ids.to(self.get_device()), attention_mask=None)
                cz = self.model.text_projection(const_embeddings.last_hidden_state)
                cz_pooled = self.model.text_projection(outputs.pooler_output)
                cz = cz / torch.norm(cz_pooled.unsqueeze(1), dim=-1, keepdim=True)
                if g_sent_idx + const_n < 77:
                    z[0, g_sent_idx + const_n] = cz[0, -1]

        max_length = z.size(1)
        bs_embed, seq_len, _ = z.shape
        dependency_matrix = torch.zeros((1, max_length, max_length)).to('cuda')
        ...
        if use_fca == 1:
            ...
            token_proj = get_lal_to_clip_idx_projection(lal_tokens, clip_tokens)
            for f_dep in lal_dependencies:
                # print('token_proj[f_dep[0]], token_proj[f_dep[1]]', token_proj[f_dep[0]], token_proj[f_dep[1]])
                dependency_matrix[0, token_proj[f_dep[0]], token_proj[f_dep[1]]] = 1
            dependency_matrix = dependency_matrix.to(self.get_device())
            dependency_matrix = dependency_matrix.half()
            dependency_matrix = dependency_matrix.repeat(1, n_samples, 1)
            dependency_matrix = dependency_matrix.view(bs_embed * n_samples, seq_len, -1)
            col_idx = dependency_matrix[0].max(dim=1)[0].nonzero()
            random.seed(seed)
            for j in range(dependency_matrix.shape[1]):
                if j not in col_idx:
                    dependency_matrix[0, j, 0] = 10

        return z, dependency_matrix
