...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None, focused_attention_mask=None, use_fca=1):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTransformer):
                x = layer(x, context, focused_attention_mask, use_fca)
            else:
                x = layer(x)
        return x
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, focused_attention_mask = None, use_fca = 1, y=None,**kwargs):
        """
        ...
        for module in self.input_blocks:
            h = module(h, emb, context, focused_attention_mask, use_fca)
            hs.append(h)
        h = self.middle_block(h, emb, context, focused_attention_mask, use_fca)
...
class TimestepEmbedSequentialExtended(nn.Sequential, TimestepBlock):
    def forward(self, x, emb, context=None, focused_attention_mask = None, use_fca =1, which_attn=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTransformer):
...
class UNetModel2D(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, focused_attention_mask = None, use_fca=1):
        hs = []
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
        emb = self.time_embed(t_emb)
...
class UNetModelVD(nn.Module):
    ...
    def forward(self, x, timesteps, context, xtype='image', ctype='prompt', focused_attention_mask=None, use_fca=1):
        hs = []
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
...
class UNetModel2D_Next(nn.Module):
    ...
    def forward(self, x, timesteps, context, focused_attention_mask, use_fca):
        hs = []
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
        emb = self.time_embed(t_emb)
